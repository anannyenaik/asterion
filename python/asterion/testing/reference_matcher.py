...
This module re-implements Asterion's matching and order-state semantics in plain
Python data structures, from the same contract the C++ engine implements.
...
This is a test oracle. It implements the subset of semantics the tests compare
against and nothing beyond it.
"""
...
#: Either matcher's report type. The C++ ``ExecutionReport`` comes from the
#: bindings; :class:`ReferenceReport` is this module's pure-Python mirror.
AnyReport: TypeAlias = "ExecutionReport | ReferenceReport"


def canonical_report_tuple(report: AnyReport) -> tuple:
