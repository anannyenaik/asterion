...
TOOLS_AND_SCRIPTS = (ROOT / "scripts",)
...
def test_schema_declares_version_one() -> None:
    assert _schema()["schema_version"] == 1, "event-log schema version drift"
...
def test_csv_schema_manifest_matches_writer_and_parser(tmp_path: Path) -> None:
    ...
    assert csv_schema["columns"] == csv_schema["required_columns"], (
        "CSV required-column manifest drifted"
    )
    assert ",".join(csv_schema["columns"]) == csv_schema["header"], (
        "CSV column-order manifest drifted"
    )
...
def test_binary_schema_manifest_matches_header_record_layout_and_enums(tmp_path: Path) -> None:
    ...
    for name, expected in event_values.items():
        ...
        assert asterion.market_event_type_to_string(getattr(asterion.MarketEventType, name)) == name, (
            f"CSV event type spelling drifted for MarketEventType.{name}"
        )
...
def test_schema_manifest_fixture_checksums_and_semantic_roundtrips(tmp_path: Path) -> None:
    ...
    for fixture in schema["fixtures"]:
        ...
        if fixture["format"] == "csv":
            ...
            assert header == schema["csv"]["header"], (
                f"CSV column drift in fixture {fixture['path']}"
            )
        else:
            ...
            assert payload[:8].decode("ascii") == schema["binary"]["magic_ascii"], (
                f"binary header drift in fixture {fixture['path']}"
            )
            assert int.from_bytes(payload[8:10], "little") == schema["schema_version"], (
                f"binary schema version drift in fixture {fixture['path']}"
            )
    ...
    for fixture in schema["fixtures"]:
        ...
        assert [_event_tuple(event) for event in _read_checked(csv_roundtrip, "csv").events] == by_path[
            fixture["path"]
        ], f"writer/reader semantic drift in CSV round-trip for {fixture['path']}"
        assert [_event_tuple(event) for event in _read_checked(bin_roundtrip, "binary").events] == by_path[
            fixture["path"]
        ], f"writer/reader semantic drift in CSV-to-binary round-trip for {fixture['path']}"
...
def test_schema_manifest_matches_python_generators() -> None:
    ...
    assert schema["csv"]["columns"] == generator.CSV_HEADER, (
        "CSV column drift in scripts/generate_synthetic_events.py"
    )
    assert generator.BINARY_MAGIC.decode("ascii") == schema["binary"]["magic_ascii"], (
        "binary header drift in scripts/generate_synthetic_events.py"
    )
    assert schema["schema_version"] == generator.BINARY_VERSION, (
        "binary schema version drift in scripts/generate_synthetic_events.py"
    )
    assert schema["binary"]["header_size_bytes"] == generator.BINARY_HEADER_SIZE
    assert schema["binary"]["record_size_bytes"] == generator.BINARY_RECORD_SIZE
    assert generator.BINARY_RECORD.size == schema["binary"]["record_size_bytes"]
    assert schema["enums"]["MarketEventType"] == generator.EVENT_TYPE_IDS, (
        "enum drift in scripts/generate_synthetic_events.py"
    )
    assert schema["enums"]["Side"] == generator.SIDE_IDS, (
        "enum drift in scripts/generate_synthetic_events.py"
    )

    assert schema["enums"]["MarketEventType"]["Add"] == binance.ET_ADD
    assert schema["enums"]["MarketEventType"]["Cancel"] == binance.ET_CANCEL
    assert schema["enums"]["MarketEventType"]["Replace"] == binance.ET_REPLACE
    assert schema["enums"]["MarketEventType"]["Snapshot"] == binance.ET_SNAPSHOT
    assert schema["enums"]["Side"]["None"] == binance.SIDE_NONE
    assert schema["enums"]["Side"]["Buy"] == binance.SIDE_BUY
    assert schema["enums"]["Side"]["Sell"] == binance.SIDE_SELL
