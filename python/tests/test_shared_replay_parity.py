...
Grouped replay (``replay_by_symbol`` / ``aggregate_by_symbol``) is the default.
These tests demonstrate, for the tested deterministic cases, that the opt-in
shared multi-symbol path agrees with it, and that the
``describe_replay_parity`` diagnostic reports a clean match. They mirror the C++
parity contract exercised here and are not a proof of parity
for all workloads.
"""
