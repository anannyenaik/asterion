...
then reports parity and SPSC stats. Deterministic single-thread replay is the
default; the SPSC path is opt-in and exists for evaluating that pipeline.
...
def main() -> int:
    ...
    parser.add_argument(
        "--drop-on-full",
        action="store_true",
        help="Lossy overload shedding; dropped events break replay parity.",
    )
