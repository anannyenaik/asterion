...
``scripts/capture_binance_depth.py`` (or a hand-curated fixture) and produces
...
order identity. Synthetic identity is an ingestion convenience, not a
reconstruction of the venue's real order flow.
...
Capture envelope: ``scripts/capture_binance_depth.py`` wraps each fetched REST
...
class _Normaliser:
    ...
    @staticmethod
    def _to_ticks(value: object, scale: int) -> int | None:
        try:
            return int((Decimal(str(value)) * scale).to_integral_value())
        except (InvalidOperation, ValueError, TypeError):
            return None

    def _best_bid_ask(self, symbol_id: int) -> tuple[int | None, int | None]:
    ...
    def _handle_snapshot(self, raw_index: int, msg: dict, symbol: str, ts_ns: int) -> None:
        ...
        for level_key in [k for k in self._active if k[0] == symbol_id]:
            self._active.pop(level_key, None)
...
def write_event_logs(
    events: Iterable[NormEvent], csv_output: str | None = None, binary_output: str | None = None
) -> None:
