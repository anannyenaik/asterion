...
* Offline commands (``benchmark-summary``, ``benchmark-compare``,
  ...
  ``audit-verify`` re-implements the risk-audit checksum chain in Python rather
  than calling the bindings, so a committed audit log can be checked against an
  implementation that did not write it.
* Native-backed commands (``replay-checksums``, ``diagnostics``, ``per-symbol``,
  ``replay-parity``, ``shared-fuzz``, ``convert``, ``risk-exposure``,
  ``portfolio-risk``, ``audit-manifest``, ``audit-manifest-verify`` and
  ``onnx-status``) import the ``asterion`` bindings lazily and therefore require
...
def cmd_audit_manifest(args: argparse.Namespace) -> int:
    ...
    if result.ok and args.output is not None and not asterion.write_audit_manifest(
        args.output, result.manifest
    ):
...
def cmd_convert(args: argparse.Namespace) -> int:
    """Rewrite an event log between the CSV and binary encodings of schema v1.

    Both encodings carry the same canonical fields, so the event checksum is
    invariant across the conversion. That is what makes this safe to use for
    regenerating the committed ``.bin`` fixtures from their ``.csv`` twins:
    compare the reported checksum with the one the CSV replay already produces.
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    convert = subparsers.add_parser(
        "convert",
        help="Convert an event log between the CSV and binary encodings.",
        description=(
            "Convert an event log between the CSV and binary encodings of schema v1. "
            "The event checksum is invariant across the conversion, so it can be used "
            "to regenerate a committed binary fixture from its CSV twin and confirm "
            "the two encodings still agree."
        ),
    )
    convert.add_argument("--input", required=True, type=Path)
    convert.add_argument("--output", required=True, type=Path)
    convert.add_argument("--input-format", default="auto", choices=["auto", "csv", "binary"])
    convert.add_argument("--output-format", default="auto", choices=["auto", "csv", "binary"])
