...
#: One generated event row: integers plus the event-type and side names.
EventField = int | str
Row = list[EventField]
...
def pack_binary_event(row: Row) -> bytes:
...
def adversarial_lifecycle_rows(args: argparse.Namespace) -> list[Row]:
    rows: list[Row] = []
    next_order_id = 1
    next_trade_id = 1
    cycle = 0
...
def main() -> None:
    ...
    if output_format == "csv":
        text_handle = open(args.output, "w", newline="", encoding="utf-8")
        handle = text_handle
        writer = csv.writer(text_handle)
        writer.writerow(CSV_HEADER)

        def write_event(row: Row) -> None:
            writer.writerow(row)

    else:
        binary_handle = open(args.output, "wb")
        handle = binary_handle
        binary_handle.write(BINARY_HEADER)

        def write_event(row: Row) -> None:
            binary_handle.write(pack_binary_event(row))
