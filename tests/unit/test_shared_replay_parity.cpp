...
// replay path. Grouped replay (replay_by_symbol) is the default;
...
//
// The contract under test: for the same event stream, grouped and shared replay
// must report the same symbol set, match on every per-symbol entry, and agree on
// the combined book and aggregate checksums. Repeating the comparison must also
// reproduce the same checksums, so the parity itself is deterministic.
