...
| [inference_event_loop_cost_report_2026_06_01.md](inference_event_loop_cost_report_2026_06_01.md) | Systems cost of inserting inference (feature extraction + LinearModel + policy gate) into the deterministic replay event loop; new measured hot-path-with-inference row vs the inference-free hot path | Local representative (new run + transcribed component/ONNX rows) | **ONNX Runtime** only for the optional ONNX rows (replay-loop+ONNX `not measured`) | Plumbing only; tiny 12-event fixture; +0 allocs / +~800 ns p50 on this run; no predictive/profitability claim; not portable |
