...
| [linux_performance_evaluation_2026_06_01.md](linux_performance_evaluation_2026_06_01.md) | **WSL2 Linux** perf-counter + latency-distribution evidence: 1M SPSC steady-state, 1M std-vs-pooled hot path, inference replay-loop, `perf stat -d` counters + `perf record` hotspots | Local representative (**WSL2**) | None (ONNX not built on Linux → ONNX replay row recorded as skipped) | WSL2 (not native/cloud Linux); virtualized PMU (no LLC events, multiplexed); uncontrolled turbo; `Full`-validation replay does not scale to 1M (uses `Light`) |
| [linux_performance_evaluation_2026_05_31.md](linux_performance_evaluation_2026_05_31.md) | Larger-corpus standard-vs-pooled evaluation (Windows/MSYS2) + ready-to-run Linux `perf` helper | Local representative; perf counters were collected later (see 2026-06-01 report) | None | Windows/MSYS2 run; counter values not fabricated when `perf` was unavailable |
| [perf_profile.md](perf_profile.md) | Linux `perf` profiling: WSL2 counters, hotspots, cache/branch reading, std-vs-pooled and SPSC interpretation, methodology | Local representative (**WSL2**) | None | WSL2 virtualized PMU; `-O3` partial call graphs; pinned SPSC record is a cost probe |
