...
// Opt-in concurrent replay pipeline: one deterministic concurrency boundary
// between market-data replay and event processing.
//
// A producer thread publishes preloaded MarketDataEvent records, in order, into
// a bounded single-producer/single-consumer queue. A consumer thread drains them,
// in order, and applies the same ReplayEngine processing path (book, validation,
// diagnostics) as the single-thread path, producing a bit-identical ReplayResult.
// Bit-identical output makes the concurrency boundary measurable without changing
// behaviour.
//
// Single-thread replay (ReplayEngine::replay_events) remains the default. This
// mode is opt-in and exists to make the boundary explicit and benchmarkable.
//
// Backpressure policy:
