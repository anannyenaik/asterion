...
| Fuzz-driven robustness testing for parser/replay/matching/audit inputs | Implemented but optional + **manual CI only** | [FUZZING.md](../FUZZING.md); `cpp/fuzz/`; `cpp/fuzz/corpus/`; [.github/workflows/fuzz-smoke.yml](../.github/workflows/fuzz-smoke.yml). Bounded libFuzzer + ASan/UBSan robustness evidence only; **not** production safety, real-exchange correctness, live-trading validation, security certification or production-HFT validation. |
...
| Matching engine cross-checked against an independent Python reference matcher (golden + fixed-seed random flows; reports, final L2 and report checksum compared) | Implemented + tested | [reference_matcher.md](reference_matcher.md); [`python/asterion/testing/reference_matcher.py`](../python/asterion/testing/reference_matcher.py), [`python/asterion/testing/cross_check.py`](../python/asterion/testing/cross_check.py); `python/tests/test_reference_matcher_golden.py`, `python/tests/test_reference_matcher_property.py`. Specification-style test oracle for the documented contract; **not** production-exchange correctness, real-exchange completeness or live-trading validation. L3 FIFO validated indirectly via the trade-report sequence (bindings expose aggregate L2). |
...
| Claim | Classification | Primary evidence |
| --- | --- | --- |
| Ubuntu-based reproducible devcontainer path for Release, C++ tests, Python tests, demo and sanitizers | Documented reviewer/development path; locally Docker-validated | [reproducible_dev_environment.md](reproducible_dev_environment.md), [`.devcontainer/Dockerfile`](../.devcontainer/Dockerfile), [`.devcontainer/devcontainer.json`](../.devcontainer/devcontainer.json). Dockerfile + documented Docker-only path validated on one local Docker Desktop Linux-container installation on 2026-06-05: GCC Release C++, Python/pytest/demo and Clang ASan/UBSan passed. Dev Container CLI unavailable, so `devcontainer up` was not separately exercised. Installs no ONNX Runtime and runs no heavy benchmark automatically. One local reviewer/development validation only; not a portable or production guarantee. |
...
| Performance deep-dive optimisation narrative (baseline node book → profiled hotspots → opt-in pooled book → before/after allocation + latency/throughput, parity, remaining bottlenecks) | Implemented + benchmarked (local) | [docs/performance_deep_dive.md](performance_deep_dive.md); measured case study under disclosed conditions, transcribes existing reports only (no new numbers), Durham HPC primary + Win/MSYS2/WSL2 historical, pooled book opt-in, all claim boundaries preserved |
