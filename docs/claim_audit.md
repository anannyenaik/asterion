...
| Event-loop inference cost: replay → L3 → L2 → caller-owned feature extraction → LinearModel → measured policy gate, alongside strategy + risk, measured vs the inference-free hot path | Implemented + benchmarked (local) | `benchmarks/benchmark_main.cpp` (`hot_path_binary_replay_l3_l2_inference_strategy_risk`); [reports/inference_event_loop_cost_report_2026_06_01.md](../reports/inference_event_loop_cost_report_2026_06_01.md). Caller-owned inference stage adds **0** steady-state allocations on top of the node-based book; plumbing only, no decisioning/alpha/profitability claim; tiny 12-event fixture, local only; replay-loop+ONNX `not measured` |
